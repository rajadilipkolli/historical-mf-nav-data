...
class DailyNavHealthServiceTest {
    ...
    void isHealthy_shouldReturnTrue_whenDatabaseIsAccessibleAndHasData() {
        ...
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM schemes", Integer.class)).thenReturn(100);
    ...
    void checkHealth_shouldReturnHealthyStatus_whenAllChecksPass() {
        // Arrange
        when(jdbcTemplate.queryForObject("SELECT 1", Integer.class)).thenReturn(1);
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM schemes", Integer.class)).thenReturn(500);
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM nav", Integer.class)).thenReturn(10000);
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM securities", Integer.class)).thenReturn(800);
        when(jdbcTemplate.queryForObject("SELECT MAX(date) FROM nav", String.class)).thenReturn("2025-07-12");
        when(jdbcTemplate.queryForObject("SELECT MIN(date) FROM nav", String.class)).thenReturn("2020-01-01");
    ...
    void checkHealth_shouldDetectStaleData() {
        ...
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM nav", Integer.class)).thenReturn(10000);
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM securities", Integer.class)).thenReturn(800);
        when(jdbcTemplate.queryForObject("SELECT MAX(date) FROM nav", String.class)).thenReturn("2025-06-01"); // Old date
        when(jdbcTemplate.queryForObject("SELECT MIN(date) FROM nav", String.class)).thenReturn("2020-01-01");
    ...
    void getStatistics_shouldReturnCompleteStatistics() {
        ...
        when(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM securities", Integer.class)).thenReturn(800);
        when(jdbcTemplate.queryForObject("SELECT MAX(date) FROM nav", String.class)).thenReturn("2025-07-12");
        when(jdbcTemplate.queryForObject("SELECT MIN(date) FROM nav", String.class)).thenReturn("2025-01-01");
