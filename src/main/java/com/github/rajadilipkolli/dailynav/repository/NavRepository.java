...
public class NavRepository {
    ...
    private static final RowMapper<Nav> NAV_ROW_MAPPER = new RowMapper<Nav>() {
        @Override
     ...
     */
    public List<Nav> findBySchemeCodeAndDateBetween(Integer schemeCode, LocalDate startDate, LocalDate endDate) {
        String sql = "SELECT scheme_code, date, nav FROM nav WHERE scheme_code = ? AND date BETWEEN ? AND ? ORDER BY date DESC";
    ...
    public Nav findLatestBySchemeCode(Integer schemeCode) {
        String sql = "SELECT scheme_code, date, nav FROM nav WHERE scheme_code = ? ORDER BY date DESC LIMIT 1";
    ...
    public Nav findBySchemeCodeAndDateOnOrBefore(Integer schemeCode, LocalDate date) {
        String sql = "SELECT scheme_code, date, nav FROM nav WHERE scheme_code = ? AND date <= ? ORDER BY date DESC LIMIT 1";
