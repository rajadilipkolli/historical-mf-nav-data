...
public class DatabaseInitializer {
    ...
    private void logDatabaseStats() {
        try {
            Integer schemeCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM schemes", Integer.class);
            Integer navCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM nav", Integer.class);
            Integer securityCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM securities", Integer.class);
            
            logger.info("Database loaded with {} schemes, {} NAV records, and {} securities", 
                       schemeCount, navCount, securityCount);
    ...
    private void loadSqlScript() throws IOException {
        ...
        if (!resource.exists()) {
            throw new RuntimeException("funds.sql not found in classpath. Please ensure the data file is included in the JAR.");
        }
        ...
        try (BufferedReader reader = new BufferedReader(
                    ...
                    if (!statement.isEmpty()) {
                        try {
                            ...
                            if (properties.isDebug()) {
                                logger.warn("Failed to execute statement at line {}: {}", lineCount, e.getMessage());
                            }
            ...
            logger.info("SQL data loaded successfully. Executed {} statements from {} lines", executedStatements, lineCount);
        }
    }
    ...
    private void createIndexes() {
        ...
        try {
            ...
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS \"nav-main\" ON \"nav\" (\"date\",\"scheme_code\")");
            
            // Index by scheme code separately to get NAV for all dates
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS \"nav-scheme\" ON \"nav\" (\"scheme_code\")");
            
            // Index all securities by scheme_code for joins with NAV table
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS \"securities-scheme\" ON \"securities\" (\"scheme_code\")");
            
            // Index all securities by isin for metadata information
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS \"securities-isin\" ON \"securities\" (\"isin\")");
